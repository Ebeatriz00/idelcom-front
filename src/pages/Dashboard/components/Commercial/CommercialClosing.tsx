import { useDashboardMetricsClosing } from "@/sharedKernel/hooks/dashboard/useDashboardCommercial";
import { Loader2, TrendingUp, FilterX, Table as TableIcon } from "lucide-react";
...
// MODIFICADO: Lógica dinámica de años (Igual que en Probabilidad y Evolución)
const CURRENT_YEAR = new Date().getFullYear();
const YEARS_OPTION = [CURRENT_YEAR - 1, CURRENT_YEAR, CURRENT_YEAR + 1];

export const DashboardClosingChart = ({ usersId, quarter, year }: Props) => {
  ...
  const { data, isLoading } = useDashboardMetricsClosing(usersId, quarter, selectedYear);

  const { chartData, tableRows, columns, uniqueStateIds, stateMeta } = useMemo(() => {
    let filteredData = data || [];
    if (activeMonth) {
        filteredData = filteredData.filter((d: any) => d.month === activeMonth);
    }

    const stateMeta: Record<number, { label: string; color: string }> = {};
    
    // Aquí el componente leerá automáticamente los colores de la BD para ID 11 y 19
    filteredData.forEach((item: any) => {
      const id = item.stateOpportunityId;
      if (!stateMeta[id]) {
        stateMeta[id] = {
            label: item.stateName || `Estado ${id}`, 
            color: item.stateColor || "#94a3b8" // Usa el color del SP o gris por defecto
        };
      }
    });
    ...
    let uniqueMonths = Array.from(new Set(filteredData.map((d: any) => d.month))).sort((a: any, b: any) => a - b);
    ...
    const chartData = uniqueMonths.map((month: any) => {
      ...
      uniqueStateIds.forEach((stateId: number) => {
        const matches = filteredData.filter((d: any) => d.month === month && d.stateOpportunityId === stateId);
    ...
    const tableRows = uniqueStateIds.map((stateId: number) => {
      ...
      uniqueMonths.forEach((month: any) => {
        const matches = filteredData.filter((d: any) => d.stateOpportunityId === stateId && d.month === month);
        const total = matches.reduce((acc: number, curr: any) => acc + curr.totalAmount, 0);
    ...
    return { chartData, tableRows, columns: uniqueMonths, uniqueStateIds, stateMeta };
  ...
  const footerStateClass = activeMonth
    ? "w-1/2 py-3 px-4 text-slate-800 text-right bg-emerald-50 border-r border-emerald-200"
    : "w-[200px] min-w-[200px] max-w-[200px] py-3 px-4 text-slate-800 text-right sticky left-0 bg-emerald-50 z-10 border-r border-emerald-200";
      ...
      <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 bg-white shrink-0">
        <div className="flex items-center gap-3">
            ...
            <p className="text-[11px] text-slate-400 font-medium">
                Comparativa Ganadas vs Perdidas ({selectedYear})
            </p>
            </div>
        </div>
        ...
        {chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              />
              {uniqueStateIds.map((stateId: number) => {
                  const meta = stateMeta[stateId];
                  return (
                    <Bar
                        key={stateId}
            ...
            <table className="w-full text-xs border-collapse rounded-lg overflow-hidden border border-slate-300 shadow-sm">
                <thead>
                    <tr>
                        ...
                        {columns.length === 0 && <th className={`py-3 px-4 bg-slate-50 border-b border-slate-300 ${activeMonth ? 'w-1/2' : 'w-auto'}`}></th>}
                    </tr>
                </thead>
                <tbody className="bg-white">
                    {tableRows.map((row: any, index: number) => (
                        <tr key={row.id} className={`transition-colors border-b border-slate-300 last:border-0 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'} hover:bg-emerald-50/50`}>
                    ...
                    <tr className="bg-emerald-50/50 font-bold border-t-2 border-emerald-200">
                        ...
                        {columns.map((month: any) => {
                            const totalCol = tableRows.reduce((acc: number, row: any) => acc + (row[month] || 0), 0);
                            return (
                                <td key={month} className={`${activeMonth ? 'w-1/2' : ''} text-right py-3 px-4 text-emerald-800 font-mono border-r border-emerald-200`}>
                                    {currencyFormatter(totalCol)}
                                </td>
                            );
                        })}
                        {columns.length === 0 && <td className={`py-3 px-4 border-r border-emerald-200 ${activeMonth ? 'w-1/2 bg-emerald-50' : ''}`}></td>}
                    </tr>
                </tbody>
            </table>
        </div>
      </div>
      <div className="h-1.5 w-full bg-gradient-to-r from-emerald-400 via-green-500 to-teal-500"></div>
    </div>
  );
};