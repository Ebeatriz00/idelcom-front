...
import { accountPlanGuards } from "../../utils/guards";

type PaginationState = { pageIndex: number; pageSize: number };

export function AccountPlanTable({
  data,
  total,
  pageCount,
  pagination,
  ...
  loading,
  canEditAccPlan = true,
  canEditStatusAccPlan = true,
  canExportAccPlan = true,
}: {
  data: AccountPlanResponseDto[];
  total: number;
  pageCount: number;
  ...
  onEdit: (row: AccountPlanResponseDto) => void;
  onToggleStatus: (row: AccountPlanResponseDto) => void;
  onDelete: (row: AccountPlanResponseDto) => void;
  onVisibleCountChange?: (n: number) => void;
  search: string;
  onSearchChange: (q: string) => void;
  loading?: boolean;
  canEditAccPlan?: boolean;
  canEditStatusAccPlan?: boolean;
  canExportAccPlan?: boolean;
}) {
  const columns = useMemo<ColumnDef<AccountPlanResponseDto, any>[]>(
    () => [
      buildSelectColumn<AccountPlanResponseDto>(),
      {
      ...
      },
      {
        ...
        header: "Tipo",
        meta: { className: "hidden sm:table-cell", label: "Tipo" },
      },
      {
        accessorKey: "accountPlanCount",
        meta: {
        ...
        cell: ({ row }) => (
          <span className="inline-flex items-center gap-1 text-gray-700">
            {row.original.accountPlanCount ?? 0}
          </span>
        ),
      },

      {
        ...
        cell: ({ row }) => {
          ...
          const inUse = accountPlanGuards.isInUse(row.original);
          const _canToggle = accountPlanGuards.canToggle(row.original);
    ...
    [onEdit, onToggleStatus, onDelete, canEditAccPlan, canEditStatusAccPlan],
  );

  const colsExport: ColumnSpec<AccountPlanResponseDto>[] = [
    { label: "Código", value: (r) => r.accountCode },
    { label: "Descripción", value: (r) => r.accountName },
    { label: "Tipo", value: (r) => r.accountType },
    {
      label: "Estado",
  ...
  const opts = {
    filePrefix: "Plan de cuenta",
    title: "Reporte de Plan de cuenta",
  };
  return (
    <DataTable<AccountPlanResponseDto>
      data={data}
      columns={columns}
      total={total}
      pageCount={pageCount}
