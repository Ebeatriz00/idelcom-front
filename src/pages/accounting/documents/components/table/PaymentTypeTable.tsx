...
import { paymentTypeGuards } from "../../utils/guards";
...
export function PaymentTypeTable({
  ...
  onSearchChange,
  canEditPayType = true,
  canEditStatusPayType = true,
  canExportPayType= true,
}: {
  data: PaymentTypeResponseDto[];
  total: number;
  pageCount: number;
  ...
  onEdit: (row: PaymentTypeResponseDto) => void;
  onToggleStatus: (row: PaymentTypeResponseDto) => void;
  onDelete: (row: PaymentTypeResponseDto) => void;
  onVisibleCountChange?: (n: number) => void;
  search: string;
  onSearchChange: (q: string) => void;
  canEditPayType?: boolean;
  canEditStatusPayType?: boolean;
  canExportPayType?: boolean;
}) {
  const columns = useMemo<ColumnDef<PaymentTypeResponseDto, any>[]>(
    () => [
      buildSelectColumn<PaymentTypeResponseDto>(),
      {
...

      {
        ...
        cell: ({ row }) => {
          ...
          const inUse = paymentTypeGuards.isInUse(row.original);
          const _canToggle = paymentTypeGuards.canToggle(row.original);
    ...
    [onEdit, onToggleStatus, onDelete, canEditPayType, canEditStatusPayType]
  );

  const colsExport: ColumnSpec<PaymentTypeResponseDto>[] = [
    { label: "Tipo comprobante", value: (r) => r.description },
  ...
  return (
    <DataTable<PaymentTypeResponseDto>
      data={data}
      columns={columns}
      total={total}
      pageCount={pageCount}
