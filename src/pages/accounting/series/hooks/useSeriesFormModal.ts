  ...
  async function submit(dto: SeriesUpsertDto) {
    try {
      ...
      const payload = {
        ...
        // El valor de 'used' viene del formulario y se asegura que no sea nulo
        used: (dto.used?.trim() ?? "").toUpperCase(),
      };
      
      if (!payload.used) {
        throw new Error("El campo 'Indicador de Uso' es obligatorio.");
      }
