...
import { exportCSV, exportExcel, exportPdf, statusToBool, } from "@/sharedKernel";
...
import { TaxAffTypeGuards } from "../../utils/guards";

type PaginationState = { pageIndex: number; pageSize: number };

export function TaxAffTypeTable({
  data,
  total,
  pageCount,
  pagination,
  ...
  onSearchChange,
  canEditTaxAff = true,
  canEditStatusTaxAff = true,
  canExportTaxAff = true,
}: {
  data: TaxAffTypeResponseDto[];
  total: number;
  pageCount: number;
  ...
  onEdit: (row: TaxAffTypeResponseDto) => void;
  onToggleStatus: (row: TaxAffTypeResponseDto) => void;
  onDelete: (row: TaxAffTypeResponseDto) => void;
  onVisibleCountChange?: (n: number) => void;
  search: string;
  onSearchChange: (q: string) => void;
  canEditTaxAff?: boolean;
  canEditStatusTaxAff?: boolean;
  canExportTaxAff?: boolean;
}) {
  const columns = useMemo<ColumnDef<TaxAffTypeResponseDto, any>[]>(
    () => [
      buildSelectColumn<TaxAffTypeResponseDto>(),
      {
      ...
      },
      {
        accessorKey: "code",
        header: "Código",
        meta: { className: "hidden sm:table-cell", label: "Código" },
      },
      {
        accessorKey: "taxAffCount",
        meta: {
...

      {
        ...
        cell: ({ row }) => {
          ...
          const inUse = TaxAffTypeGuards.isInUse(row.original);
          const _canToggle = TaxAffTypeGuards.canToggle(row.original);
    ...
    [onEdit, onToggleStatus, onDelete, canEditTaxAff, canEditStatusTaxAff]
  );

  const colsExport: ColumnSpec<TaxAffTypeResponseDto>[] = [
    { label: "Tipo afectación Tributaria", value: (r) => r.description },
  ...
  const opts = {
    filePrefix: "Tipo Comprobante de pago",
    title: "Reporte de Tipo Comprobante de pago",
  };
  return (
    <DataTable<TaxAffTypeResponseDto>
      data={data}
      columns={columns}
      total={total}
      pageCount={pageCount}
      ...
      searchPlaceholder="Buscar... (nombre, código)"
    />
  );
}
