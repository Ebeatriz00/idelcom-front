import type { PermissionsUpsertDto } from "@/application/dtos/configurations/Permissions/Permissions.dto";
import type { PermissionsResponseDto } from "@/application/dtos/configurations/Permissions/PermissionsResponse.dto";
import {
  createPermissions,
  updatePermissions,
} from "@/infrastructure/api-clients/configurations/permissions/permission.client";
import {
  closeAlert,
  showApiError,
  showLoading,
  showSuccess,
  type GlobalResponse,
} from "@/sharedKernel";
import { qkPermissions } from "@/sharedKernel/hooks/permissions/usePermissions";
import { useMutation, useQueryClient } from "@tanstack/react-query";

function patchPermissionsAfterUpdate(
  qc: ReturnType<typeof useQueryClient>,
  predicate: (it: PermissionsResponseDto) => boolean,
  updater: (it: PermissionsResponseDto) => PermissionsResponseDto
) {
  const caches = qc.getQueriesData<{ items: PermissionsResponseDto[] }>({
    queryKey: qkPermissions.lists(),
    exact: false,
  });
  for (const [key, data] of caches) {
    if (!data?.items) continue;
    const next = {
      ...data,
      items: data.items.map((it) => (predicate(it) ? updater(it) : it)),
    };
    qc.setQueryData(key, next);
  }
}

export const usePermissionsMutations = () => {
  const qc = useQueryClient();

  const createMut = useMutation<
    GlobalResponse,
    unknown,
    Omit<PermissionsUpsertDto, "permissionsId">
  >({
    mutationFn: createPermissions,
    onMutate: () => showLoading("Registrando nuevo módulo..."),
    onSuccess: async (res) => {
      closeAlert();
      if (res.status === 1) {
        await showSuccess("Éxito", res.message);
        await qc.invalidateQueries({
          queryKey: qkPermissions.lists(),
          exact: false,
          refetchType: "active",
        });
      } else {
        await showApiError(
          { response: { data: res } },
          "No se pudo registrar."
        );
      }
    },
    onError: async (e) => {
      closeAlert();
      await showApiError(e, "Error registrando.");
    },
  });

  const updateMut = useMutation<GlobalResponse, unknown, PermissionsUpsertDto>({
    mutationFn: updatePermissions,
    onMutate: () => showLoading("Actualizando módulo..."),
    onSuccess: async (res, vars) => {
      closeAlert();
      if (res.status === 1) {
        await showSuccess("Éxito", res.message);
        if (vars.permissionsId) {
          patchPermissionsAfterUpdate(
            qc,
            (it) => it.permissionsId === vars.permissionsId,
            (it) => ({
              ...it,
              permissionsName: vars.permissionsName,
              permissionsCode: vars.permissionsCode,
              permissionsDescription: vars.permissionsDescription,
            })
          );
        }
      } else {
        await showApiError(
          { response: { data: res } },
          "No se pudo actualizar."
        );
      }
    },
    onError: async (e) => {
      closeAlert();
      await showApiError(e, "Error actualizando.");
    },
  });

  return { createMut, updateMut };
};
