...
import { profileGuards } from "../../utils/guards";
...
export function ProfilesTable({
  ...
  onEdit: (row: ProfilesResposeDto) => void;
  onToggleStatus: (row: ProfilesResposeDto) => void;
  onDelete: (row: ProfilesResposeDto) => void;
  search: string;
  onSearchChange: (q: string) => void;
  onVisibleCountChange?: (n: number) => void;
}) {
  const columns = useMemo<ColumnDef<ProfilesResposeDto, any>[]>(
    () => [
      buildSelectColumn<ProfilesResposeDto>(),
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="inline-flex items-center gap-1 text-gray-700">
            <Users2 className="size-4" /> {row.original.usersCount ?? 0}
          </span>
        ),
      },
      ...
      // Acciones
      {
        ...
        cell: ({ row }) => {
          ...
          const inUse = profileGuards.isInUse(row.original);
          const _canToggle = profileGuards.canToggle(row.original);
  ...
  const colsExport: ColumnSpec<ProfilesResposeDto>[] = [
    { label: "Nombre", value: (r) => r.name },
    { label: "Descripción", value: (r) => r.description ?? "" },
    { label: "Usuarios", value: (r) => r.usersCount ?? 0 },
    {
      label: "Estado",
  ...
  const opts = { filePrefix: "Perfiles", title: "Reporte de Perfiles" };
  return (
    <DataTable<ProfilesResposeDto>
      data={data}
      columns={columns}
      total={total}
      pageCount={pageCount}
