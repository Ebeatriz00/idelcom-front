...
import { contactTypeGuards } from "../../utils/guards";

export function ContactTypeTable({
  data,
  total,
  pageCount,
  pagination,
  ...
  search,
  onSearchChange,
  canExportContactType = true, 
  canEditContactType = true,
  canEditStatusContactType = true,
}: {
  data: ContactTypeResponseDto[];
  total: number;
  pageCount: number;
  ...
  onEdit: (row: ContactTypeResponseDto) => void;
  onToggleStatus: (row: ContactTypeResponseDto) => void;
  onDelete: (row: ContactTypeResponseDto) => void;
  onVisibleCountChange?: (n: number) => void;
  search: string;
  onSearchChange: (q: string) => void;
  canExportContactType?: boolean;
  canEditContactType?: boolean;
  canEditStatusContactType?: boolean;
}) {
  const columns = useMemo<ColumnDef<ContactTypeResponseDto, any>[]>(
    () => [
      buildSelectColumn<ContactTypeResponseDto>(),
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="inline-flex items-center gap-1 text-gray-700">
            {row.original.contactTypeCount ?? 0}
          </span>
        ),
      },

      {
        ...
        cell: ({ row }) => {
          ...
          const inUse = contactTypeGuards.isInUse(row.original);
          const _canToggle = contactTypeGuards.canToggle(row.original);
    ...
    [onEdit, onToggleStatus, onDelete, canEditContactType, canEditStatusContactType]
  );

  const colsExport: ColumnSpec<ContactTypeResponseDto>[] = [
    { label: "Tipo de Contacto", value: (r) => r.description },
    {
      label: "Estado",
  ...
  const opts = {
    filePrefix: "Tipo de contacto",
    title: "Reporte de Tipo de contacto",
  };
  return (
    <DataTable<ContactTypeResponseDto>
      data={data}
      columns={columns}
      total={total}
      pageCount={pageCount}
      ...
      searchPlaceholder="Buscar... (tipo contacto)"
    />
  );
}
