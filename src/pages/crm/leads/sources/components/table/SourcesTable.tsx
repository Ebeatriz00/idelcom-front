...
import { leadsSourcesGuards } from "../../utils/guards";
...
export function SourcesTable({
  ...
  onSearchChange,
  canEditSources = true,
  canEditStatusSources = true,
  canExportSources = true,
}: {
  data: LeadsSourcesResponseDto[];
  total: number;
  pageCount: number;
  ...
  onEdit: (row: LeadsSourcesResponseDto) => void;
  onToggleStatus: (row: LeadsSourcesResponseDto) => void;
  onDelete: (row: LeadsSourcesResponseDto) => void;
  onVisibleCountChange?: (n: number) => void;
  search: string;
  onSearchChange: (q: string) => void;
  canEditSources?: boolean;
  canEditStatusSources?: boolean;
  canExportSources?: boolean;
}) {
  const columns = useMemo<ColumnDef<LeadsSourcesResponseDto, any>[]>(
    () => [
      buildSelectColumn<LeadsSourcesResponseDto>(),
      {
        accessorKey: "description",
        meta: { className: "truncate", label: "Fuentes" },
        header: ({ column }) => (
          <button
            type="button"
          ...
          >
            Fuentes <ArrowUpDown className="size-3.5" />
          </button>
        ),
        cell: ({ row }) => {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="inline-flex items-center gap-1 text-gray-700">
            {row.original.leadsSourcesCount ?? 0}
          </span>
        ),
      },

      {
        ...
        cell: ({ row }) => {
          ...
          const inUse = leadsSourcesGuards.isInUse(row.original);
          const _canToggle = leadsSourcesGuards.canToggle(row.original);
    ...
    [onEdit, onToggleStatus, onDelete, canEditSources, canEditStatusSources]
  );

  const colsExport: ColumnSpec<LeadsSourcesResponseDto>[] = [
    { label: "Fuentes", value: (r) => r.description },
    {
      label: "Estado",
  ...
  const opts = {
    filePrefix: "Fuentes leads",
    title: "Reporte de Fuentes leads",
  };
  return (
    <DataTable<LeadsSourcesResponseDto>
      data={data}
      columns={columns}
      total={total}
      pageCount={pageCount}
