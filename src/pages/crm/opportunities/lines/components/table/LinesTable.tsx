...
import { businessLineGuards } from "../../utils/guards";
...
export function LinesTable({
  ...
  search,
  onSearchChange,
  canExportBLines = true,
  canEditBLines = true,
  canEditStatus = true,
}: {
  data: BusinessLineResponseDto[];
  total: number;
  pageCount: number;
  ...
  onEdit: (row: BusinessLineResponseDto) => void;
  onToggleStatus: (row: BusinessLineResponseDto) => void;
  onDelete: (row: BusinessLineResponseDto) => void;
  onVisibleCountChange?: (n: number) => void;
  search: string;
  onSearchChange: (q: string) => void;
  canEditBLines?: boolean;
  canExportBLines?: boolean;
  canEditStatus?: boolean;
}) {
  const columns = useMemo<ColumnDef<BusinessLineResponseDto, any>[]>(
    () => [
      buildSelectColumn<BusinessLineResponseDto>(),
      {
        accessorKey: "descLine",
...

      {
        ...
        cell: ({ row }) => {
          ...
          const inUse = businessLineGuards.isInUse(row.original);
          const _canToggle = businessLineGuards.canToggle(row.original);
    ...
    [onEdit, onToggleStatus, onDelete, canEditBLines, canEditStatus]
  );

  const colsExport: ColumnSpec<BusinessLineResponseDto>[] = [
    { label: "Descripción", value: (r) => r.descLine },
    {
      label: "Estado",
  ...
  const opts = {
    filePrefix: "Lineas de negocio",
    title: "Reporte de Lineas de negocio",
  };
  return (
    <DataTable<BusinessLineResponseDto>
      data={data}
      columns={columns}
      total={total}
      pageCount={pageCount}
