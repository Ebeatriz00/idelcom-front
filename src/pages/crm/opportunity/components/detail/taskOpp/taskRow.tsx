...
export function TaskRow({
  ...
  const { canEditStateTasksComm, canEditPriorityTasksComm } =
    useCrmOpporPerms();
    ...
    <li className="rounded-2xl border px-3 py-3 sm:px-4 hover:bg-gray-50 transition">
      <div className="flex flex-col sm:flex-row sm:items-start gap-3">
        <button
          ...
          disabled={isDone}
          className={cn(
            "text-gray-600 sm:mt-0.5",
            isDone ? "opacity-50 cursor-not-allowed" : "hover:text-gray-800"
          )}
          title={
            isDone ? "La tarea ya está completada" : "Marcar como completada"
          }
        >
          {isDone ? (
