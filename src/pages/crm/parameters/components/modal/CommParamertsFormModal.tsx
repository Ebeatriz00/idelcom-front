...
import { CommercialParametersForm } from "./CommParamertsForm";
...
type Props = {
  ...
  onClose: () => void;
  onSubmit: (dto: CommercialParametersUpsertDto) => Promise<void>;
  saving: boolean;
};

export function CommercialParametersFormModal({
  open,
  title,
  loadingDetail,
  defaultValues,
  ...
  const formId = "CommercialParameters-form";
  return (
    <Modal
      title={title}
      size="xl"
    ...
    >
      {loadingDetail ? (
        ...
        <CommercialParametersForm
          key={defaultValues.commercialParametersId ?? "new"}
