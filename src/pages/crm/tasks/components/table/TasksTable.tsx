...
import { tasksGuards } from "../../utils/guards";
...
export function TasksTable({
  ...
  onEdit: (row: TasksResponseDto) => void;
  onToggleStatus: (row: TasksResponseDto) => void;
  onVisibleCountChange?: (n: number) => void;
  search: string;
  onSearchChange: (q: string) => void;
}) {
  const columns = useMemo<ColumnDef<TasksResponseDto, any>[]>(
    () => [
      buildSelectColumn<TasksResponseDto>(),
      {
        accessorKey: "title",
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const inUse = tasksGuards.isInUse(row.original); 
          const _canToggle = tasksGuards.canToggle(row.original);
  ...
  const colsExport: ColumnSpec<TasksResponseDto>[] = [
    { label: "Título", value: (r) => r.title },
    { label: "Oportunidad", value: (r) => r.opporDescription },
