...
export function SsomaDocumentTypeTable({
  data,
  total,
  pageCount,
  pagination,
  ...
  const columns = useMemo<ColumnDef<SsomaDocumentTypeResponseDto, any>[]>(
    () => [
      buildSelectColumn<SsomaDocumentTypeResponseDto>(),
      {
        accessorKey: "ssomaDocumentTypeDesc",
        meta: { className: "truncate", label: "Tipo de Documento" },
        header: ({ column }) => (
          <button
            type="button"
          ...
          >
            Tipo de Documento <ArrowUpDown className="size-3.5" />
          </button>
        ),
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-900 truncate">
                {row.original.ssomaDocumentTypeDesc}
              </span>
              {isActive ? (
  ...
  const colsExport: ColumnSpec<SsomaDocumentTypeResponseDto>[] = [
    { label: "Tipo de Documento", value: (r) => r.ssomaDocumentTypeDesc },
    { label: "Estado", value: (r) => (statusToBool(r.status) ? "Activo" : "Inactivo") },
  ];

  const opts = {
    filePrefix: "Tipos Documento SSOMA",
    title: "Reporte de Tipos de Documento SSOMA",
  };
  
  return (
    <DataTable<SsomaDocumentTypeResponseDto>
      data={data}
      columns={columns}
      total={total}
      pageCount={pageCount}
