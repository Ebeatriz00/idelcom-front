import type { OptionItem, TasksProjectUpsertDto } from "@/application";
...
const schema = z.object({
  tasksId: z.number().optional(),
  projectToken: z.string().nullable().optional(),
...
export function TasksProjectForm({
  defaultValues,
  onSubmit,
  saving,
  formId,
  ...
  defaultValues?: Partial<TasksProjectUpsertDto>;
  onSubmit: (dto: TasksProjectUpsertDto) => void;
  saving?: boolean;
  formId?: string;
  showActions?: boolean;
  autofocus?: boolean;
  ...
  const {
    ...
    mode: "onChange",
    defaultValues: {
      tasksId: defaultValues?.tasksId,
      projectToken: defaultValues?.projectToken ?? null,
  ...
  const stateTaskOptions = useMemo(
    () => stateTaskResp?.items ?? [],
    [stateTaskResp]
  );
  ...
  const priorityStateOptions = useMemo(
    () => priorityStateResp?.items ?? [],
    [priorityStateResp]
  );
    ...
    <form
      ...
      onSubmit={handleSubmit((data) => {
        onSubmit({
          ...
          projectToken: data.projectToken ?? null,
          stateTaskId: data.stateTaskId,
          workerId: data.workerId,
          priorityStateId: data.priorityStateId,
          endDate: data.endDate,
          time: data.time,
          numPercPro: defaultValues?.numPercPro ?? 0,
        });
      })}
      className="space-y-5"
    >
      <div className="grid grid-cols-1 gap-x-6 gap-y-4 md:grid-cols-2">
            ...
            render={({ field, fieldState }) => (
              <>
                <UpperInput
                  ...
                  autoFocus={autofocus}
                  placeholder="EJ: REVISAR AVANCE DEL PROYECTO"
                  maxLength={100}
        ...
        {/* RESPONSABLE (SIN RESTRICCIONES DE PERMISOS) */}
        <div className="col-span-full">
          <label className="mb-1 block text-xs font-medium text-gray-600">
          ...
          <Controller
            ...
            render={({ field, fieldState }) => {
              const currentVal =
                workerOptions.find((o) => Number(o.value) === field.value) ||
                (field.value && workerLabel
                  ? { value: field.value, label: workerLabel }
                  : null);

              return (
                <>
                  <SearchSelect
                    useOptions={useWorkerProyectOptions}
